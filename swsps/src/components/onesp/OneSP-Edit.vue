          ...
          <div>
            <div class="imgs-div imgs-img1" v-show="imgshow.img1">
              <input
                ...
                id="input-img1"
                style="display:none;"
                @change="handleImgUploadChg('img1')"
              />
              <div class="place-img">
            ...
            <div class="imgs-div imgs-img2" v-show="imgshow.img2">
              <input
                ...
                id="input-img2"
                style="display:none;"
                @change="handleImgUploadChg('img2')"
              />
              <div class="place-img">
  ...
  methods: {
    deletesps: function() {
      this.$confirm("此操作将永久删除该备件, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$http
            .delete("/api/sps/one/" + this.onesp.id)
            .then(resp => {
              if (resp.data === "Done") {
                this.$message({
                  showClose: true,
    ...
    handlerDeleteUpload: function(which) {
      ...
      this.img_deleted[which] = true;
      let file = document.querySelector("#input-" + which);
      file.value = null;
    },
    cancelEdit: function() {
      this.$emit("chgmod", "sp-imf");
    ...
    comfirmEdit: function() {
      ...
      // if delete the img send the delete msg
      if (this.img_deleted.img1) {
        uploadData.append("delete_img1", "y");
      } else {
        uploadData.append("delete_img1", "n");
      }

      if (this.img_deleted.img2) {
        uploadData.append("delete_img2", "y");
      } else {
        uploadData.append("delete_img2", "n");
      }

      let file1 = document.querySelector("#input-img1");
      if (file1.value) {
        uploadData.append("img1", file1.files[0]);
      }
      ...
      this.$http
        .post("/api/sps/update", uploadData, {
          method: "post",
