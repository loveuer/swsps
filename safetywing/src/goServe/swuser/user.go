...
func Get(uname string) (User, error) {
	sentence := fmt.Sprintf("select id, username, realname, profile_icon, last_login from swuser where username = '%s'", uname)
...
func getUserPasswordByUserName(username string) string {
	sentence := fmt.Sprintf("select password from swuser where username='%s'", username)
	row := conn.QueryRow(sentence)
